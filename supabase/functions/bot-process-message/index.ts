...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    console.log("History query: fetched=" + (historyData ? historyData.length : 0) + " reversed to chronological order")
    ...
    if (chronoHistory) {
      ...
      console.log("History filter: original=" + chronoHistory.length + " after generic filter=" + historyAfterGeneric.length + " after media filter=" + filteredHistory.length)
    }
    ...
    console.log("User message added to apiMessages: original=" + message_content.substring(0, 50) + " transformed=" + transformed + " final_length=" + finalUserMessage.length);
