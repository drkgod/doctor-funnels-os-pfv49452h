...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if ((recentBotMessages ?? 0) > 5) {
      return new Response(
        JSON.stringify({ skipped: true, message: 'Limite de resposta do bot atingido. Aguarde.' }),
        {
          status: 200,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        },
    ...
    if (!botConfig) {
      return new Response(
        JSON.stringify({ skipped: true, message: 'Nenhum bot ativo para este tenant.' }),
        {
          status: 200,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        },
    ...
    if (!apiKeyData) {
      return new Response(
        JSON.stringify({ skipped: true, message: 'Chave API nao configurada para o provedor.' }),
        {
          status: 200,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        },
