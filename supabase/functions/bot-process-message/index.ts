                ...
                if (!date) {
                  ...
                  if (date < today) {
                    ...
                    if (!isOpen) {
                      ...
                      let q = supabaseAdmin.from('appointments').select('datetime_start, datetime_end').eq('tenant_id', tenant_id).gte('datetime_start', `${date}T00:00:00Z`).lte('datetime_start', `${date}T23:59:59Z`).neq('status', 'cancelled');
                ...
                if (!new_date || !new_time) {
                  ...
                  if (target_id) {
                    ...
                    if (overlapping && overlapping.length > 0) {
                      ...
                      let q = supabaseAdmin.from('appointments').select('datetime_start, datetime_end').eq('tenant_id', tenant_id).gte('datetime_start', `${new_date}T00:00:00Z`).lte('datetime_start', `${new_date}T23:59:59Z`).neq('status', 'cancelled');
