...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (decryptError || !decryptedToken) {
      return new Response(JSON.stringify({ error: 'Erro ao acessar chave OpenAI.' }), {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }
    ...
    if (!oaRes.ok) {
      const errorData = await oaRes.json()
    ...
    return new Response(JSON.stringify({ error: 'Erro interno no servico de IA.' }), {
      status: 500,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    })
  }
